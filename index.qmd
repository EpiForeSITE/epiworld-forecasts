...
lfmcmc_simulation_fun <- function(params) {
  ...
  infected_transitions <- hist_matrix[hist_matrix$state_to == "Infected" & hist_matrix$state_from != "Infected", c("date", "counts")]
  # infected_transitions <- hist_matrix[grep("Infected", hist_matrix$state_to), c("date","counts")]
  # - We aggregate the 3 values for each day (transition to Infected from each SIR state)
  # - We also drop the last day of data because the model returns data for (model_ndays + 1) days
  infected_cases <- head(aggregate(x = infected_transitions,
    by = list(infected_transitions$date),
    FUN = sum), -1)

  return(as.double(infected_cases$counts))
}
...
Our forecast is represented by this distribution:
```{r}
...
forecast_dist <- cbind(c(1:model_ndays), forecast_dist)
forecast_dist <- as.data.frame(forecast_dist)
sample_names <- paste0("sample_", 1:forecast_sample_n)
colnames(forecast_dist) <- c("day", sample_names)
forecast_dist <- forecast_dist %>% pivot_longer(cols = sample_names,
  names_to = "sample",
  values_to = "counts")

ggplot(forecast_dist, aes(x = day, y = counts)) +
  geom_line(aes(color = sample)) +
  labs(x = "Day (step)", y = "Forecasted Cases") +
  theme_classic() +
