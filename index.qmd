...
lfmcmc_simulation_fun <- function(params) {
  ...
  infected_cases <- hist_matrix[grep("Infected", hist_matrix$state), c("date", "counts")]
  return(infected_cases)
}

# Expects dat to include case counts in second column
lfmcmc_summary_fun <- function(dat) {
  # Extract summary statistics from the data
  cases <- dat[, 2]
  time_to_peak <- which.max(cases)
  # TODO: compute as % of pop
  size_of_peak <- cases[time_to_peak]
  sd_cases <- sd(cases)
