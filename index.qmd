...
model_seed     <- 112
model_ndays    <- 90
model_n        <- 5000    # model population size
n_samples      <- 2000   # number of LFMCMC iterations
...
lfmcmc_summary_fun <- function(case_counts) {
  ...
  c(
    ...
    mean_cases,
    sd_cases,
    spline(case_counts, n = n_splines)$y
  )
}
...
res <- lapply(seq_along(param_names), \(i) {
  data.frame(
    step  = seq_len(nrow(res)),
    param = param_names[i],
    value = res[, i]
  )
}) |> do.call(what = "rbind")

ggplot(res, aes(x = step, y = value)) +
