...
n_samples <- 6000   # number of LFMCMC iterations
...
lfmcmc_proposal_fun <- function(params_prev) {
  ...
  params_1_to_5 <- plogis(
    qlogis(params_prev[1:5]) +
      rnorm(length(params_prev[1:5]), mean = 0, sd = 0.05)
  )

  params_6_to_7 <- params_prev[6:7] +
    rnorm(2, mean = 0, sd = 0.05)
...
# Save to CSV
res_csv <- res
colnames(res_csv) <- param_names
write.csv(res_csv, file = "build/html/lfmcmc_model_parameters.csv")
stats_csv <- get_accepted_stats(lfmcmc_model)
colnames(stats_csv) <- stats_names
write.csv(stats_csv, file = "build/html/lfmcmc_model_statistics.csv")

# Process data into useful format
res <- lapply(seq_along(param_names), \(i) {
  data.frame(
...
# Extract contact and recovery rates
res_r_rate <- res[res$param == param_names[1], ]
res_c_rate <- res[res$param == param_names[6:7], ]
...
res_t_rate <- res[res$param == t_params_used, ]
...
init_r_c_names <- c(param_names[6],
  param_names[7],
  param_names[1])
init_r_c_df <- data.frame(param = init_r_c_names,
  value = init_r_c_values)

init_t_df <- data.frame(param = t_params_used,
  value = t_values_used)
