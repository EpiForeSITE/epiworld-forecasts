...
Our forecast is represented by the following distribution:
...
forecast_sample_mean <- apply(forecast_dist, 1, mean)
forecast_sample_median <- apply(forecast_dist, 1, median)

# Compute confidence interval
confint_fun <- function(x, conf_level = 0.95) {
  res <- t.test(x, conf.level = conf_level)
  res <- res$conf.int
  attributes(res) <- NULL
  res
}

fc_95conf_int <- t(apply(forecast_dist, 1, confint_fun))
fc_50conf_int <- t(apply(forecast_dist, 1,
  confint_fun, conf_level = 0.5))

# Combine observed data with sample median for plotting forecast
observed_df <- data.frame(
  day = forecast_data$Date[60:90],
...
sample_df <- data.frame(
  day = forecast_data$Date[90] + 1:14,
  counts = forecast_sample_mean,
  observed = FALSE,
  lb_95 = fc_95conf_int[, 1],
  ub_95 = fc_95conf_int[, 2],
  lb_50 = fc_50conf_int[, 1],
  ub_50 = fc_50conf_int[, 2]
)
...
cbb_palette <- c("#56B4E9", "#E69F00", "#000000", "#009E73")
...
ggplot(forecast_df,
  ...
  geom_ribbon(aes(ymin = lb_50, ymax = ub_50),
    ...
    alpha = 0.5,
    na.rm = TRUE) +
  geom_point(aes(y = counts, color = observed)) +
  geom_line(aes(y = counts, color = observed)) +
