...
lfmcmc_simulation_fun <- function(params) {
  ...
  change_c_and_t_rates <- function(model) {
    ...
    current_season <- get_date_season(forecast_data$Date[current_model_day + 1])
    ...
    current_model_day <<- current_model_day + 1L

    invisible(model)
  }

  globalevent_fun(change_c_and_t_rates, name = "Change Contact and Transmission Rates") |>
  ...
  hist_matrix <- get_hist_total(ef_model)
  # - model returns 91 values, drop last value
  infected_cases <- head(hist_matrix[grep("Infected", hist_matrix$state), "counts"], -1)
...
lfmcmc_model <- LFMCMC(model_sirconn) |>
